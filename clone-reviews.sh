...
for i in rabbitmq mysql clustercheck haproxy redis; do (cd $i-tht && link_repo $THT); done
...
for i in rabbitmq mysql clustercheck haproxy redis; do (cd $i-pp && link_repo $PP); done
